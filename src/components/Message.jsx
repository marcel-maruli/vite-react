...
const imageFiles = import.meta.glob('../assets/MessageImage*.jpeg');
    ...
    const loadImages = async () => {
      const loadedImages = await Promise.all(
        Object.keys(imageFiles)
          .sort((a, b) => {
            // Extract the number from filenames (e.g., MessageImage1.jpeg'); -> 1)
            const aNum = parseInt(a.match(/MessageImage(\d+)\.jpeg;/)?.[1] || 0, 10);
            const bNum = parseInt(b.match(/MessageImage(\d+)\.jpeg;/)?.[1] || 0, 10);
